...
// فقط subtitle با index: 0 و کانتکست اطرافش
const context = subtitles.filter(sub => sub.index >= -1 && sub.index <= 1).map(sub => ({
  index: sub.index,
  text: sub.text
}));
...
(async () => {
  try {
    ...
    if (!output) throw new Error('جوابی از Gemini دریافت نشد');

    console.log('\nترجمه subtitle با index 0:\n');
    console.log(output);
  } catch (err) {
    console.error('خطا در ترجمه:', err.response?.data || err.message);
  }
})();